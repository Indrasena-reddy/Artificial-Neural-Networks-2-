...
def accuracy(model,indices):
    ...
    p=0
    for i in (indices):
        pred = model(torch.unsqueeze(mnist_test[i][0],0))
...
def seq(s):
    ...
    for f in range(len(s)):
        seql.append(nn.Conv2d(in_channels=indim, out_channels=s[f], kernel_size=3,padding = 1))
        indim = s[f]
        seql.append(nn.ReLU())  
        seql.append(nn.MaxPool2d(kernel_size=2, stride=2))
    #fully connected layers
    seql.append(nn.Flatten())
    seql.append(nn.Linear(s[-1]*((int(28/2**len(s)))**2), 500, bias = True))
    seql.append(nn.ReLU())  
    seql.append(nn.Linear(500, 10, bias = True))    
    return seql 

def cnn_train(hidden_dims, epochs, batch_size, learning_rate, cuda, plots):
